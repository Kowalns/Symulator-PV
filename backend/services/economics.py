...
# --- Degradacja baterii i projekcja 25-letnia ---

# Degradacja baterii: 20% utraty pojemnosci po 10 latach (liniowo)
DEGRADACJA_BATERII_PROCENT_ROK = 2.0  # 2% rocznie = 20% po 10 latach

# Koszt wymiany magazynu (orientacyjny)
KOSZT_WYMIANY_MAGAZYNU_ZL_KWH = 2000.0  # ~2000 PLN/kWh pojemnosci

# Rok wymiany magazynu (typowo po 10-12 latach)
ROK_WYMIANY_MAGAZYNU = 12


def oblicz_degradacje_baterii(rok_eksploatacji: int,
                              degradacja_roczna_procent: float = DEGRADACJA_BATERII_PROCENT_ROK) -> float:
    """
    Oblicza wspolczynnik pojemnosci baterii po degradacji.

    Model: liniowy spadek 2%/rok (20% po 10 latach).
    Po wymianie (rok 12+) pojemnosc wraca do 100%.

    Parametry:
        rok_eksploatacji: ktory rok eksploatacji (1-25)
        degradacja_roczna_procent: roczna degradacja baterii [%]

    Zwraca:
        Wspolczynnik pojemnosci (0.0-1.0), gdzie 1.0 = pelna pojemnosc
    """
    degradacja = degradacja_roczna_procent / 100.0

    if rok_eksploatacji <= ROK_WYMIANY_MAGAZYNU:
        # Przed wymiana - degradacja liniowa
        wspolczynnik = 1.0 - degradacja * (rok_eksploatacji - 1)
        return max(0.3, wspolczynnik)  # Minimum 30% pojemnosci
    else:
        # Po wymianie - nowa bateria, degradacja od nowa
        lata_po_wymianie = rok_eksploatacji - ROK_WYMIANY_MAGAZYNU
        wspolczynnik = 1.0 - degradacja * (lata_po_wymianie - 1)
        return max(0.3, wspolczynnik)


def oblicz_projekcje_ekonomiczna_25_lat(
        oszczednosc_roczna_rok1_zl: float,
        koszt_inwestycji_zl: float,
        pojemnosc_magazynu_kwh: float = 0.0,
        degradacja_paneli_procent: float = 0.5,
        degradacja_baterii_procent: float = DEGRADACJA_BATERII_PROCENT_ROK,
        inflacja_taryf_procent: float = 5.0,
        koszty_serwisowe_roczne_zl: float = 800.0,
        koszt_wymiany_falownika_zl: float = 6500.0,
        rok_wymiany_falownika: int = 12) -> Dict:
    """
    Oblicza projekcje ekonomiczna na 25 lat z degradacja baterii.

    Uwzglednia:
    - Degradacja paneli PV (0.5%/rok)
    - Degradacja baterii (2%/rok, wymiana po ~12 latach)
    - Inflacja taryf energetycznych (5%/rok - skraca ROI)
    - Koszty serwisowe (500-1500 PLN/rok)
    - Wymiana falownika co 10-15 lat (~5-8 tys. PLN)
    - Koszt wymiany magazynu po degradacji

    Parametry:
        oszczednosc_roczna_rok1_zl: oszczednosc w pierwszym roku [PLN]
        koszt_inwestycji_zl: calkowity koszt instalacji [PLN]
        pojemnosc_magazynu_kwh: pojemnosc magazynu [kWh] (0 = brak)
        degradacja_paneli_procent: degradacja paneli [%/rok]
        degradacja_baterii_procent: degradacja baterii [%/rok]
        inflacja_taryf_procent: roczny wzrost cen energii [%]
        koszty_serwisowe_roczne_zl: roczne koszty serwisowe [PLN]
        koszt_wymiany_falownika_zl: koszt wymiany falownika [PLN]
        rok_wymiany_falownika: rok wymiany falownika

    Zwraca:
        Slownik z projekcja roczna i podsumowaniem
    """
    projekcja = []
    skumulowana_oszczednosc = -koszt_inwestycji_zl
    rok_zwrotu = None

    for rok in range(1, 26):
        # Degradacja paneli PV
        wsp_paneli = (1.0 - degradacja_paneli_procent / 100.0) ** (rok - 1)

        # Degradacja baterii (wplywa na udzial magazynu w oszczednosciach)
        wsp_baterii = oblicz_degradacje_baterii(rok, degradacja_baterii_procent)

        # Inflacja taryf - im drozsza energia, tym wieksza oszczednosc z PV
        wsp_inflacji = (1.0 + inflacja_taryf_procent / 100.0) ** (rok - 1)

        # Oszczednosc w danym roku (uwzglednia degradacje paneli i inflacje)
        # Zakladamy ze ~30% oszczednosci pochodzi z magazynu (proporcja zalezy od konfiguracji)
        udzial_magazynu = 0.3 if pojemnosc_magazynu_kwh > 0 else 0.0
        udzial_pv = 1.0 - udzial_magazynu

        oszczednosc_pv = oszczednosc_roczna_rok1_zl * udzial_pv * wsp_paneli * wsp_inflacji
        oszczednosc_mag = oszczednosc_roczna_rok1_zl * udzial_magazynu * wsp_baterii * wsp_inflacji
        oszczednosc_rok = oszczednosc_pv + oszczednosc_mag

        # Koszty w danym roku
        koszty_rok = koszty_serwisowe_roczne_zl

        # Wymiana falownika
        wymiana_falownika = False
        if rok == rok_wymiany_falownika:
            koszty_rok += koszt_wymiany_falownika_zl
            wymiana_falownika = True

        # Wymiana magazynu
        wymiana_magazynu = False
        koszt_wymiany_mag = 0.0
        if pojemnosc_magazynu_kwh > 0 and rok == ROK_WYMIANY_MAGAZYNU:
            koszt_wymiany_mag = pojemnosc_magazynu_kwh * KOSZT_WYMIANY_MAGAZYNU_ZL_KWH
            koszty_rok += koszt_wymiany_mag
            wymiana_magazynu = True

        # Bilans roku
        bilans_rok = oszczednosc_rok - koszty_rok
        skumulowana_oszczednosc += bilans_rok

        if rok_zwrotu is None and skumulowana_oszczednosc >= 0:
            rok_zwrotu = rok

        projekcja.append({
            "rok": rok,
            "oszczednosc_brutto_zl": round(oszczednosc_rok, 2),
            "koszty_serwisowe_zl": round(koszty_serwisowe_roczne_zl, 2),
            "wymiana_falownika_zl": round(koszt_wymiany_falownika_zl if wymiana_falownika else 0.0, 2),
            "wymiana_magazynu_zl": round(koszt_wymiany_mag, 2),
            "bilans_netto_zl": round(bilans_rok, 2),
            "skumulowana_oszczednosc_zl": round(skumulowana_oszczednosc, 2),
            "wsp_degradacji_paneli": round(wsp_paneli, 4),
            "wsp_degradacji_baterii": round(wsp_baterii, 4),
            "wsp_inflacji_taryf": round(wsp_inflacji, 4),
        })

    # Podsumowanie
    suma_oszczednosci = sum(p["oszczednosc_brutto_zl"] for p in projekcja)
    suma_kosztow = sum(p["koszty_serwisowe_zl"] + p["wymiana_falownika_zl"] + p["wymiana_magazynu_zl"]
                       for p in projekcja)

    return {
        "projekcja_roczna": projekcja,
        "podsumowanie": {
            "koszt_inwestycji_zl": round(koszt_inwestycji_zl, 2),
            "suma_oszczednosci_25_lat_zl": round(suma_oszczednosci, 2),
            "suma_kosztow_eksploatacji_zl": round(suma_kosztow + koszt_inwestycji_zl, 2),
            "zysk_netto_25_lat_zl": round(suma_oszczednosci - suma_kosztow - koszt_inwestycji_zl, 2),
            "rok_zwrotu_inwestycji": rok_zwrotu,
            "wymiana_magazynu_rok": ROK_WYMIANY_MAGAZYNU if pojemnosc_magazynu_kwh > 0 else None,
            "wymiana_falownika_rok": rok_wymiany_falownika,
            "degradacja_baterii_info": (
                f"Pojemnosc baterii spada o {degradacja_baterii_procent}%/rok "
                f"(do {round((1 - degradacja_baterii_procent/100 * (ROK_WYMIANY_MAGAZYNU-1)) * 100)}% po {ROK_WYMIANY_MAGAZYNU-1} latach). "
                f"Wymiana w roku {ROK_WYMIANY_MAGAZYNU}."
            ) if pojemnosc_magazynu_kwh > 0 else None,
        },
    }
